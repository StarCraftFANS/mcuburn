...
#include <mcuburnconfig.h>
#endif
#include <heximage.h>
#define FILEMODE   0
#define DEVICEMODE 1
#define NORMALWRITE 0
...
Line *readLine(FILE *inFile)
{
    ...
    if (!line)
    {
        fprintf(stderr, "heximage: no memory!\n");
        return NULL;
    }
    ...
    {
        fprintf(stderr, "heximage: can't read line length @ line %d\n", 
        	lineno);
    	free(line);
    	return NULL;
    }
    ...
    {
        fprintf(stderr, "heximage: line length %X too long @ line %d\n",
                (int)line->len, lineno);
        free(line);
        return NULL;
    }
    ...
    {
	fprintf(stderr, "heximage: can't read line offset @ line %d\n",
                lineno);
        free(line);
        return NULL;        
    }
    bp += 4; /* eour digits consumed. */
    
    if (getHexByte(bp, &line->type))
    {
        fprintf(stderr, "heximage: can't read record type @ line %d\n", 
        	lineno);
    	free(line);
    	return NULL;
    }
    ...
    for (i = 0; i < (unsigned)line->len; i++)
    {
        ...
        {
            fprintf(stderr, 
            	    "heximage: can't read data byte %u of %u @ line %d\n", 
    ...
    {
        fprintf(stderr, "heximage: can't read checksum @ line %d\n", 
        	lineno);
        free(line->data);
    	free(line);
    	return NULL;
...
int validateChecksum(Line *line)
{
    ...
    {
        fprintf(stderr, "heximage: invalid checksum %X (want %X) @ line %d\n", 
...
int writeRecord(unsigned len, unsigned offset, unsigned type,
		const Uint8 *data,int fd,int mode)
{
        FILE *outputfile;
    ...
    if(!mode)
    {
     if (RECPRINT(":%02X%04X%02X", len, offset, type) == EOF)
     {
        return -1;
     }
    
    }
    outputfile=fopen("TESTS","a");
    fprintf(outputfile,"NEW RECORD: ");
    for (i = 0; i < len; i++)
    {
        if(!mode)
        {
          if (RECPRINT("%02X", data[i]) == EOF)
          {
            return -1;
          }
        }
        ...
        {
                hexnum=((data[i])&0xff);
                filebuffer[globalindex++]=hexnum;
                writebuffer(fd,&hexnum,1);
          usleep(100);
          fprintf(outputfile," %x : ",hexnum);
        }
    }
        fprintf(outputfile," LEN : %d\n",len);
        fclose(outputfile);
    if(!mode)
    {
...
int bufferOutput(Line *line,int fd,int mode)
{
   unsigned offset = 0;
   int	    rc = 0;
   ...
   {
       rc = writeRecord(OUTPUT_CHUNK, pendingOffset, 0, &pending[offset],fd,mode);
       offset += OUTPUT_CHUNK;
...
int flushPendingData(int fd,int mode)
{
    int rc = 0;
    ...
    if (pendingLen)
    {
        rc =  writeRecord(pendingLen, pendingOffset, 0, pending,fd,mode);
...
int writeLine(Line *line,int fd,int mode)
{
    ...
    if (line->type)
    {
        ...
        if (line->type == 4)
        {
            ...
            {
            	fprintf(stderr, 
                 	"heximage: invalid extended offset record @ line %d\n",
                 	lineno);
                return -1;
            }
     	...
     	if (flushPendingData(fd,mode))
     	{
     	    return -1;
     	}
     	
     	/* Write the line as is. */
     	rc = writeRecord(line->len, line->offset, line->type, line->data,fd,mode);
    }
    else
    {
        ...
        rc = bufferOutput(line,fd,mode);
    }
    return rc;
}

int sendihx(int fd,char *argv)
{
    ...
    int  rc = 0;
     ...
     fprintf(stderr,"Writing file %s\n",argv);
        ...
        if (!inFile)
        {
            fprintf(stderr, "heximage: cannot open %s\n", 
            	     argv);
            return 1;
        }
    closeFile=1;
   ...
   // filelen=ftell(fp);
    //rewind(fp);
    filebuffer=(unsigned char *)malloc(sizeof(unsigned char)*(filelen+1));
    globalindex=0;
    ...
    while (!rc && ((line = readLine(inFile)) != NULL))
    {
        ...
        if (!rc)
        {
            rc = writeLine(line,fd,DEVICEMODE);
        }
        free(line->data);
        free(line);
    }
    ...
    if (!rc && !feof(inFile))
    {
        ...
        fprintf(stderr, "heximage: aborting after %d lines.\n", lineno);
        rc = 1;
    }
    ...
    if (!rc)
    {
        ...
        rc = flushPendingData(fd,DEVICEMODE);
    }
    
    if (!rc)
    {
        fprintf(stderr, "heximage: read %d lines, wrote %d: OK.\n", lineno, outlineno);
    }
...
int openihx(int argc, char *argv[])
{
    FILE *inFile;
    Line *line;
    int  closeFile;
    ...
    while (!rc && ((line = readLine(inFile)) != NULL))
    {
        ...
        if (!rc)
        {
            rc = writeLine(line,NORMALWRITE,FILEMODE);
        }
    ...
    {
    	/* Just in case there's something still pending. */
        rc = flushPendingData(NORMALWRITE,FILEMODE);
    }
