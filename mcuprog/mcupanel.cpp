...
IMPLEMENT_DYNAMIC_CLASS(wxMCUPanel, wxPanel)


	BEGIN_EVENT_TABLE(wxMCUPanel, wxPanel)
	EVT_PAINT(wxMCUPanel::OnPaint)
	EVT_RIGHT_DOWN(wxMCUPanel::OnClick)
	END_EVENT_TABLE()

	wxMCUPanel::wxMCUPanel(wxWindow * parent, wxWindowID id,
						   const wxPoint & pos, const wxSize & size,
						   wxColour & clr)
:wxPanel(parent, id, pos, size, wxRAISED_BORDER | wxTAB_TRAVERSAL)
{
    wxColour fclr(255,255,255);
	//  wxBitmap  bitmap( 100, 100 );
	//wxMemoryDC    dc;
     SetForegroundColour(fclr);
...
wxMCUPanel::~wxMCUPanel()
{
}

void wxMCUPanel::OnPaint(wxPaintEvent & WXUNUSED(event))
{
#if 0
	wxPaintDC dc(this);
...
void wxMCUPanel::WriteData(int x, int y, wxString & strs, wxColour & tclr)
{
//wxDC dcobj=GetDC();
...
void wxMCUPanel::OnClick(wxMouseEvent & event)
{
